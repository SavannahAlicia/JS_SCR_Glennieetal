...
start <- list(lambda0 = exp(-5.3), sigma = exp(9.0), beta = 0.5, phi = plogis(1.6), D = exp(1.4))
...
m_sal <- JsModel$new(par_sal, barb, start) 
tmp <- m_sal$par()
...
m_sal$set_par(tmp)
m_sal$fit(list(iterlim = 1000, print.level = 2, stepmax = 10))
